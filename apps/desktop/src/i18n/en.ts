...
export const en: Translations = {
  ...
  git: {
    ...
    branchWidgetTitle: 'Current branch — open Commit',
    ...
    checkout: 'Checkout',
    newBranchFrom: name => `New Branch from '${name}'…`,
    mergeInto: (from, into) => `Merge '${from}' into '${into}'`,
    rebaseOnto: (current, target) => `Rebase '${current}' onto '${target}'`,
    rename: 'Rename…',
    delete: 'Delete',
    renameTitle: name => `Rename branch '${name}'`,
    deleteConfirm: name => `Delete branch '${name}'?`,
    cancel: 'Cancel',
    binaryFile: 'Binary file — no preview.',
    compareWith: name => `Compare with '${name}'`,
    showDiffWorkingTree: 'Show Diff with Working Tree',
  ...
  database: {
    aria: 'Database',
    title: 'Database',
    empty: 'No databases yet. Open a SQLite file to get started.',
    addConnection: 'Open database',
    removeConnection: 'Remove connection',
    refreshSchema: 'Refresh schema',
    selectConnection: 'Select a database',
    openDatabase: 'Open SQLite database',
    noTables: 'No tables.',
    newConsole: 'New SQL console',
    openData: 'Open data',
    rowRange: (start, end) => `${start}–${end}`,
    prevPage: 'Previous page',
    nextPage: 'Next page',
    queryPlaceholder: 'Write SQL — ⌘/Ctrl+Enter to run',
    run: 'Run',
    runShortcut: '⌘/Ctrl+Enter runs the statement under the caret',
    runHint: 'Run a query or open a table to see results.',
    truncated: 'truncated',
    ...
    connectionFailed: 'Could not add the connection.',
    driverMissing: pkg => `Driver '${pkg}' is not installed.`,
    installDriver: 'Install',
    installingDriver: 'Installing driver…',
    file: 'File',
    openTable: name => `Open ${name}`,
  ...
  settings: {
    ...
    customEndpointsIntro: 'Self-hosted or proxy endpoints you have added. Edit the URL, transport, and models, or remove them.',
    editEndpoint: 'Edit endpoint',
    transportAuto: 'Auto transport',
    modelCount: (count: string) => `${count} models`,
    removeEndpointTitle: 'Remove custom endpoint?',
    removeEndpointConfirm: (name: string) => `Remove "${name}"? This deletes its configuration from config.yaml.`,
    endpointRemoved: (name: string) => `Removed "${name}".`,
  ...
  onboarding: {
    ...
    transportAuto: 'Auto-detect from URL',
    transportChat: 'OpenAI Chat Completions (/v1/chat/completions)',
    transportAnthropic: 'Anthropic Messages (/v1/messages)',
    transportResponses: 'OpenAI Responses / Codex (/v1/responses)',
    modelsLabel: 'Models (optional)',
    modelsPlaceholder: 'gpt-4o, claude-sonnet-4, gemini-2.5-pro',
    modelsHint: 'Comma- or line-separated. Leave blank to auto-discover from /v1/models. First is the default.',
    forceOauthLabel: 'Force Claude Code OAuth adaptation',
    forceOauthHint: 'Use for a proxy that forwards to a Claude Code OAuth upstream. Injects the Claude Code identity and renames MCP tools (mcp__) so Anthropic accepts the request.',
    couldNotSave: 'Could not save credential.',
    connecting: 'Connecting',
    update: 'Update',
    flowSubtitles: {
  ...
  rightSidebar: {
    ...
    deleteConfirm: name => `Move “${name}” to the trash? This can be undone from your system trash.`,
