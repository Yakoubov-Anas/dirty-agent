...
export const ja = defineLocale({
  ...
  git: {
    ...
    title: 'コミット',
    refresh: '更新',
    notARepo: 'このフォルダーは Git リポジトリではありません。',
    clean: '変更はありません。',
    noDiff: 'プレビューする変更はありません。',
    ...
    branchWidgetTitle: '現在のブランチ — コミットを開く',
    searchPlaceholder: 'ブランチとアクションを検索',
    ...
    checkout: 'チェックアウト',
    newBranchFrom: name => `'${name}' から新しいブランチ…`,
    mergeInto: (from, into) => `'${from}' を '${into}' にマージ`,
    rebaseOnto: (current, target) => `'${current}' を '${target}' にリベース`,
    rename: '名前を変更…',
    delete: '削除',
    renameTitle: name => `ブランチ '${name}' の名前を変更`,
    deleteConfirm: name => `ブランチ '${name}' を削除しますか？`,
    cancel: 'キャンセル',
    binaryFile: 'バイナリファイル — プレビューなし。',
    compareWith: name => `'${name}' と比較`,
    showDiffWorkingTree: 'ワーキングツリーとの差分を表示',
    compareTitle: (base, target) => `${base} … ${target}`,
  ...
  rightSidebar: {
    ...
    revealFile: 'ツリーで開いているファイルを表示',
    copyPath: 'パスをコピー',
    copyRelativePath: '相対パスをコピー',
    revealInOS: 'ファイルマネージャーで表示',
    previewUnavailable: 'プレビューは利用できません',
