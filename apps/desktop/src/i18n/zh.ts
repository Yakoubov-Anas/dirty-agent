...
export const zh: Translations = {
  ...
  git: {
    ...
    noBranches: '未找到分支。',
    checkout: '检出',
    newBranchFrom: name => `从 '${name}' 新建分支…`,
    mergeInto: (from, into) => `将 '${from}' 合并到 '${into}'`,
    rebaseOnto: (current, target) => `将 '${current}' 变基到 '${target}'`,
    rename: '重命名…',
    delete: '删除',
    renameTitle: name => `重命名分支 '${name}'`,
    deleteConfirm: name => `删除分支 '${name}'？`,
    cancel: '取消',
    binaryFile: '二进制文件 — 无法预览。',
    compareWith: name => `与 '${name}' 比较`,
    showDiffWorkingTree: '显示与工作树的差异',
    ...
    ahead: count => `领先 ${count} 个提交`,
    behind: count => `落后 ${count} 个提交`
  },

  database: {
    aria: '数据库',
    title: '数据库',
    empty: '还没有数据库。打开一个 SQLite 文件开始使用。',
    addConnection: '打开数据库',
    ...
    prevPage: '上一页',
    nextPage: '下一页',
    queryPlaceholder: '编写 SQL — ⌘/Ctrl+Enter 运行',
    run: '运行',
    runShortcut: '⌘/Ctrl+Enter 运行光标处的语句',
    runHint: '运行查询或打开表以查看结果。',
    truncated: '已截断',
    filterResults: '筛选结果…',
    export: '导出',
    exportCsv: '导出 CSV',
    ...
    testConnection: '测试连接',
    testOk: '连接成功',
    connectionFailed: '无法添加连接。',
    driverMissing: pkg => `未安装驱动 '${pkg}'。`,
    installDriver: '安装',
    installingDriver: '正在安装驱动…',
    file: '文件',
    openTable: name => `打开 ${name}`,
  ...
  settings: {
    ...
    customEndpointsIntro: '你添加的自托管或代理端点。可编辑 URL、传输格式和模型，或将其移除。',
    editEndpoint: '编辑端点',
    transportAuto: '自动传输',
    modelCount: (count: string) => `${count} 个模型`,
    removeEndpointTitle: '移除自定义端点？',
    removeEndpointConfirm: (name: string) => `移除“${name}”？这将从 config.yaml 删除其配置。`,
    endpointRemoved: (name: string) => `已移除“${name}”。`,
      loading: '正在加载提供方...'
    },
    sessions: {
      loading: '正在加载已归档会话…',
  ...
  onboarding: {
    ...
    modelsHint: '用逗号或换行分隔。留空则从 /v1/models 自动发现。第一个为默认模型。',
    forceOauthLabel: '强制 Claude Code OAuth 适配',
    forceOauthHint: '适用于转发到 Claude Code OAuth 上游的代理。注入 Claude Code 身份并重命名 MCP 工具（mcp__），使 Anthropic 接受请求。',
    couldNotSave: '无法保存凭据。',
    connecting: '连接中',
    update: '更新',
    flowSubtitles: {
  ...
  rightSidebar: {
    ...
    refreshTree: '刷新文件树',
    collapseAll: '折叠所有文件夹',
    revealFile: '在文件树中定位打开的文件',
    copyPath: '复制路径',
    copyRelativePath: '复制相对路径',
    revealInOS: '在文件资源管理器中显示',
    previewUnavailable: '预览不可用',
